...
local function GAMBLE_roulette(arg)
   if arg == '' then
      ChatMsg(format(
         ".:Mommarte's Casino:. --ROULETTE!-- Please type %s to join the round (type %s to leave). The buy in is 1g!",
         vars.chatEnterMsg, vars.chatWithdrawMsg))
      round.acceptEntries = true
   else
      round.multiplier = round.multiplier * tonumber(arg)
      ChatMsg(format(
         ".:Mommarte's Casino:. --ROULETTE!-- Please type %s to join the round (type %s to leave). The buy in is %sg!",
